...
#   (Get-FileHash ripsync-v0.4.0-x86_64-pc-windows-msvc.zip -Algorithm SHA256).Hash
git commit -am "ripsync 0.4.0" && git push
```
...
1. Fill `InstallerSha256` and `ProductCode` from the released MSI. The fastest
   path is `wingetcreate update beeboyd.ripsync --version 0.4.0 --urls <MSI-URL>`
...
3. Open a PR to [microsoft/winget-pkgs](https://github.com/microsoft/winget-pkgs)
   ...
   `manifests/b/beeboyd/ripsync/0.4.0/`. `wingetcreate submit` automates the PR.
